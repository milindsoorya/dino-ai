# https://github.com/zademn/mnist-mlops-learning?ref=zzun.app
...
# TRAIN_URL = urllib.parse.urljoin(BACKEND_URL, "train_auto")
PREDICT_URL = urllib.parse.urljoin(BACKEND_URL, "predict")
DELETE_URL = urllib.parse.urljoin(BACKEND_URL, "delete")


st.title("Denoising using Autoencoders")
st.subheader(
    "Train and evaluate denosing methods on your browser")
...
if page == "Train":
    ...
    st.session_state.model_type = st.selectbox(
        "Model type", options=["Linear", "CNN", "Autoencoder", "Diffusion", 'NeRF'])

    model_name = st.text_input(label="Model name", value="My Model")

    # Model selector - Linear
    if st.session_state.model_type == "Linear":
        num_layers = st.select_slider(
            label="Number of hidden layers", options=[1, 2, 3], key="linear_layers")
    ...
    # Model selector - Autoencoder
    if st.session_state.model_type == "Autoencoder":
        # hyperparameters
        encoded_dims = st.select_slider(
            label="Encoded space dimensions", options=[4, 5, 6], key="autoencoder_ouput_dimesions")

        learn_rate = st.select_slider(
            label="Select a learning rate", options=[0.1, 0.01, 0.001])

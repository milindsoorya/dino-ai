...
def train_model_task(model_name: str, hyperparams: dict, epochs: int):
    ...
    with mlflow.start_run() as run:
        ...
        model = Autoencoder(hyperparams).to(device)
        ...
        if tracking_url_type_store != "file":
            mlflow.pytorch.log_model(
                model, "Autoencoder", registered_model_name=model_name, conda_env=mlflow.pytorch.get_default_conda_env())
        else:
            mlflow.pytorch.log_model(
                model, "Autoencoder-MNIST", registered_model_name=model_name)

...
def train_model_task(model_name: str, hyperparams: dict, epochs: int):
    ...
    with mlflow.start_run() as run:
        ...
        print("Loading data...")
        train_dataloader, valid_dataloader, test_dataloader = load_mnist_data()
        ...
        encoder = Encoder(
            encoded_space_dim=hyperparams.encoded_dims, fc2_input_dim=128)
        decoder = Decoder(
            encoded_space_dim=hyperparams.encoded_dims, fc2_input_dim=128)
        params_to_optimize = [
        ...
        trainer = Trainer(encoder, decoder, params_to_optimize,
                          device=device)  # Default configs

        # Train the model
        # with mlflow.start_run() as run:
        #     history = trainer.train(epochs, train_dataloader, valid_dataloader)
        ...
        if tracking_url_type_store != "file":
            mlflow.pytorch.log_model(
                {encoder: encoder, decoder: decoder}, "encoer_decoder", registered_model_name=model_name, conda_env=mlflow.pytorch.get_default_conda_env())
        else:
            mlflow.pytorch.log_model(
                {encoder: encoder, decoder: decoder}, "encoer_decoder", registered_model_name=model_name)
